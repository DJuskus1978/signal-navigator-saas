...
// ── Supabase via raw fetch (no SDK needed) ──

async function supabaseAuth(authHeader: string): Promise<{ id: string } | null> {
...
async function cacheGet(key: string): Promise<unknown | null> {
  try {
    ...
    const res = await fetch(
      `${url}/rest/v1/stock_cache?cache_key=eq.${encodeURIComponent(key)}&expires_at=gt.${encodeURIComponent(new Date().toISOString())}&select=data&limit=1`,
...
async function cacheSet(key: string, val: unknown, mins: number) {
  try {
    const url = Deno.env.get('SUPABASE_URL')!;
    const now = new Date();
    await fetch(`${url}/rest/v1/stock_cache`, {
      method: 'POST',
      headers: {
      ...
      body: JSON.stringify({ cache_key: key, data: val, fetched_at: now.toISOString(), expires_at: new Date(now.getTime() + mins * 60000).toISOString() }),
    });
  } catch { /* ignore */ }
}
...
async function handleDetail(sym: string, apiKey: string) {
  ...
  const q = first(results[0]) as Record<string, unknown> | null;
  ...
  const grades = Array.isArray(results[10]) ? results[10] as Record<string, unknown>[] : [];
...
serve(async (req: Request) => {
  ...
  try {
    ...
    const user = await supabaseAuth(auth);
